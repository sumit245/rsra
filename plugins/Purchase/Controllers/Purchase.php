...
class Purchase extends Security_Controller
{
  ...
  private function _make_vendor_row($data)
  {
    ...
    $delete = '<li role="presentation">' . modal_anchor(get_uri("purchase/delete_vendor_modal_form"), "<i data-feather='x' class='icon-16'></i> " . app_lang('delete'), ["title" => app_lang('delete') . "?", "data-post-id" => $data->userid, "class" => "dropdown-item"]) . '</li>';
  ...
  private function _make_approval_setting_row($data)
  {
    ...
    return [
      ...
      modal_anchor(get_uri("purchase/modal_approval_setting_form"), "<i data-feather='edit' class='icon-16'></i>", ["class" => "edit", "title" => app_lang('edit_approval_setting'), "data-post-id" => $data['id']])
  ...
  public function item_modal_form()
  {
    ...
    $data['categories_dropdown'] = $this->Item_categories_model->get_dropdown_list(["title"]);
    $data['units']               = $this->Purchase_model->get_unit_add_item();
  ...
  public function delete_commodity()
  {
    ...
    if (is_array($response) && isset($response['referenced'])) {
      $this->session->setFlashdata("error_message", _l("is_referenced"));
    } elseif ($response == true) {
      $this->session->setFlashdata("success_message", _l("deleted_successfully"));
    } else {
      $this->session->setFlashdata("error_message", _l("problem_deleting"));
    }
    app_redirect('purchase/items');
  }
  ...
  public function vendor_items_table()
  {
    ...
    $result = data_tables_purchase($aColumns, $sIndexColumn, $sTable, $join, $where, [db_prefix() . 'pur_vendor.userid as userid', 'datecreate', 'description', 'commodity_code', 'title'], '', [], $this->request->getPost());
    ...
    foreach ($rResult as $aRow) {
      ...
      $options = modal_anchor(get_uri("purchase/delete_vendor_item_modal_form"), "<i data-feather='trash' class='icon-16'></i> ", ["title" => app_lang('delete') . "?", "data-post-id" => $aRow['vendor_items_id'], "class" => "btn btn-danger delete icon-btn"]);
    ...
    if ($id != '') {
      ...
      if (count($data['pur_request_detail']) > 0) {
        ...
        foreach ($data['pur_request_detail'] as $request_detail) {
          ...
          $purchase_request_row_template .= $this->Purchase_model->create_purchase_request_row_template(
            'items[' . $index_request . ']',
            $request_detail['item_code'],
            $item_text,
            $request_detail['sku_code'] ?? '', // Make
            $request_detail['sku_name'] ?? '', // Model
            $request_detail['quantity'],
            $unit_name,
            $request_detail['prd_id'],
            true
          );
        }
      }
  ...
  public function get_purchase_request_row_template()
  {
    ...
    $item_text   = $this->request->getPost('item_text');
    $item_code   = $this->request->getPost('item_code');
    $sku_code    = $this->request->getPost('sku_code');   // ✅ new
    $sku_name    = $this->request->getPost('sku_name');   // ✅ new
    $quantity    = $this->request->getPost('quantity');
    $unit_name   = $this->request->getPost('unit_name');
    $item_key    = $this->request->getPost('item_key');

    log_message('critical', 'Purchase Request Row Input: ' . json_encode([
      'name'       => $name,
      'item_text'  => $item_text,
      'item_code'  => $item_code,
      'sku_code'   => $sku_code,
    ...
    echo html_entity_decode($this->Purchase_model->create_purchase_request_row_template(
      $name,
      $item_code,
      $item_text,
      $sku_code,
   ...
   * @return [type]
   */
  public function pur_create_notification($data = [])
  {
  ...
  public function estimate($id = '')
  {
    ...
    foreach ($team_members as $team_member) {
      $data['staffs'][] = ["id" => $team_member->id, "text" => $team_member->first_name . " " . $team_member->last_name];
    }
    $data['vendors'] = $this->Purchase_model->get_vendors();
  ...
  public function view_commodity_detail($commodity_id)
  {
    ...
    $model_info         = $this->Items_model->get_details(["id" => $commodity_id, "login_user_id" => $this->login_user->id])->getRow();
  ...
  public function pur_order($id = '')
  {
    ...
    foreach ($team_members as $team_member) {
      $data['staff'][] = ["id" => $team_member->id, "text" => $team_member->first_name . " " . $team_member->last_name];
    }
  ...
  public function add_expense()
  {
    ...
    if ($recurring) {
      ...
      if (get_array_value($data, "next_recurring_date") && get_today_date() >= $data['next_recurring_date']) {
        echo json_encode(["success" => false, 'message' => app_lang('past_recurring_date_error_message_title'), 'next_recurring_date_error' => app_lang('past_recurring_date_error_message'), "next_recurring_date_value" => $data['next_recurring_date']]);
        return false;
      }
    }
  ...
  public function send_po_modal_form($po_id)
  {
    ...
    if ($po_id) {
      ...
      $contacts_options = ["user_type" => "vendor", "vendor_id" => $po_info->vendor];
      ...
      foreach ($contacts as $contact) {
        ...
        $cc_contacts_dropdown[] = ["id" => $contact->id, "text" => $contact->first_name . " " . $contact->last_name];
      }
  ...
  public function get_send_po_template($po_id = 0, $contact_id = 0, $return_type = "", $po_info = "", $contact_info = "")
  {
    ...
    if ($return_type == "json") {
      echo json_encode(["success" => true, "message_view" => $message]);
    } else {
      return [
        "message" => $message,
        "subject" => $subject,
  ...
  public function send_po()
  {
    ...
    //add invoice pdf
    array_unshift($attachments, ["file_path" => $attachement_url]);

    if (send_app_mail($contact->email, $subject, $message, ["attachments" => $attachments, "cc" => $cc_array, "bcc" => $bcc_emails])) {
      ...
      if ($files_data) {
        ...
        foreach ($files as $file) {
          delete_app_files($target_path, [$file]);
        }
      }

      echo json_encode(['success' => true, 'message' => app_lang("po_sent_message"), "po_id" => $po_id]);
    } else {
      echo json_encode(['success' => false, 'message' => app_lang('error_occurred')]);
    }
  }
  ...
  public function validate_invoices_file()
  {
  ...
  public function send_pq_modal_form($pq_id)
  {
    ...
    if ($pq_id) {
      ...
      $contacts_options = ["user_type" => "vendor", "vendor_id" => $pq_info->vendor];
      $contacts         = $this->Purchase_model->get_contact_details($contacts_options)->getResult();
  ...
  public function send_pur_quotation()
  {
    ...
    if (send_app_mail($contact->email, $subject, $message, ["attachments" => $attachments, "cc" => $cc_array, "bcc" => $bcc_emails])) {
      ...
      echo json_encode(['success' => true, 'message' => app_lang("pq_sent_message"), "pq_id" => $pq_id]);
    } else {
      echo json_encode(['success' => false, 'message' => app_lang('error_occurred')]);
    }
  }
  ...
  public function send_pr_modal_form($pr_id)
  {
    ...
    if ($pr_id) {
      ...
      $contacts_options = ["user_type" => "vendor"];
      $contacts         = $this->Purchase_model->get_contact_details($contacts_options)->getResult();
      ...
      foreach ($contacts as $contact) {
        ...
        $cc_contacts_dropdown[] = ["id" => $contact->id, "text" => $contact->first_name . " " . $contact->last_name . " - " . get_vendor_company_name($contact->vendor_id)];
      }
  ...
  public function send_pur_request()
  {
    ...
    if (send_app_mail($contact->email, $subject, $message, ["attachments" => $attachments, "cc" => $cc_array, "bcc" => $bcc_emails])) {
      ...
      echo json_encode(['success' => true, 'message' => app_lang("pr_sent_message"), "pr_id" => $pr_id]);
    } else {
      echo json_encode(['success' => false, 'message' => app_lang('error_occurred')]);
    }
  }
