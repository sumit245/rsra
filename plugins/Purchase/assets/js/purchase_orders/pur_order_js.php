...
$(function(){
  "use strict";
    ...
    $('.po_submit').on('click',function(){
      $('._pur_order_form').submit();
    });
...
function estimate_by_vendor(invoker){
  ...
  if(invoker.value != 0){
    $.post( "<?php echo get_uri('purchase/estimate_by_vendor/'); ?>" +invoker.value).done(function(response){
      ...
      <?php if(get_setting('po_only_prefix_and_number') != 1){ ?>
...
function coppy_pur_estimate(){
  ...
  if(pur_estimate != ''){
    $.post("<?php echo get_uri('purchase/coppy_pur_estimate/'); ?>"+pur_estimate).done(function(response){
        response = JSON.parse(response);
        if(response){ 
...
function client_change(el){
  ...
  $.post("<?php echo get_uri('purchase/inv_by_client'); ?>", data).done(function(response){
    response = JSON.parse(response);
    $('select[name="sale_invoice"]').html(response.html);
    $('select[name="sale_invoice"]').select2();
  });
...
function coppy_sale_invoice(){
  ...
  if(sale_invoice != ''){
    $.post("<?php echo get_uri('purchase/coppy_sale_invoice_po/'); ?>" +sale_invoice).done(function(response){
        response = JSON.parse(response);
...
function pur_get_item_row_template(name, item_name, description, quantity, unit_name, unit_price, taxname,  item_code, unit_id, tax_rate, discount, item_key, currency_rate, to_currency)  {
  ...
  var d = $.post("<?php echo get_uri('purchase/get_purchase_order_row_template'); ?>", {
    name: name,
    item_name : item_name,
