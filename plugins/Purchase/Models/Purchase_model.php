...
class Purchase_model extends Crud_model
{
   ...
   */
  public function get_vendor_table($options = [])
  {
   ...
   * @param      array  $options  The options
   */
  public function get_unit_table($options = [])
  {
   ...
   */
  public function get_item_categories_table($option = [])
  {
   ...
   */
  public function get_subgroup_table($option = [])
  {
   ...
   */
  public function get_vendor_category_table($option = [])
  {
  ...
  public function create_approval_setting_row_template($staff_data = [], $name = '', $approver = 'staff', $staff = '', $action = '', $item_key = '')
  {
    ...
    $row .= '<div class="col-md-4 d-none"><td class="approver d-none">' .
      render_select1($name_approver, $approver_data, ['name', ['label']], '', $approver, [], ["data-none-selected-text" => app_lang('approver_name')], 'no-margin') .
      '</td></div>';

    $row .= '<div class="col-md-7"><td class="staff">' .
      render_select1($name_staff, $staff_data, ['id', ['first_name', 'last_name']], 'staff_name', $staff, ['data-none-selected-text' => 'asasa'], ["data-none-selected-text" => app_lang('pur_staff_name')], 'no-margin') .
      '</td></div>';

    $row .= '<div class="col-md-4"><td class="action">' .
      render_select1($name_action, $action_data, ['name', 'label'], 'wh_action', $action, [], ["data-none-selected-text" => app_lang('wh_action')], 'no-margin') .
      '</td></div>';
   ...
   * @return [type]
   */
  public function pur_create_notification($event, $user_id, $options = [], $to_user_id = 0)
  {
  ...
  public function add_estimate($data)
  {
    ...
    log_message('critical', 'Printing Data' . print_r($data, true));
    unset($data['item_select']);
  ...
  public function create_purchase_order_row_template($name = '', $item_name = '', $item_description = '', $quantity = '', $unit_name = '', $unit_price = '', $taxname = '', $item_code = '', $unit_id = '', $tax_rate = '', $total_money = '', $discount = '', $discount_money = '', $total = '', $into_money = '', $tax_id = '', $tax_value = '', $item_key = '', $is_edit = false, $currency_rate = 1, $to_currency = '')
  {
    ...
    if ($name == '') {
      ...
      if ((float) $tax_rate_value != 0) {
        $discounted_price = (float)$unit_price - (float)$discount_money;
        $tax_money   = (float)$discounted_price * (float) $quantity * (float) $tax_rate / 100;
        $goods_money = (float) $discounted_price * (float) $quantity + (float)$tax_money;
        $amount      = (float) $discounted_price * (float) $quantity + (float) $tax_money;
      } else {
        $discounted_price = (float)$unit_price - (float)$discount_money;
        $goods_money = (float) $unit_price * (float) $quantity;
        $amount      = (float) $discounted_price * (float) $quantity;
      }
    ...
    $row .= '<td class="">' .
      render_textarea1($name_item_name, '', $item_name, ['rows' => 2, 'placeholder' => _l('pur_item_name'), 'readonly' => true]) .
      render_textarea1($name_item_description, '', $item_description, ['rows' => 2, 'placeholder' => _l('item_description')]) .
      '</td>';
    ...
    $row .= '<td class="rate ' . $tax_rate_class . '">' . $this->get_taxes_dropdown_template($name_tax_id_select, $invoice_item_taxes, 'invoice', $item_key, true, $manual) . '</td>';
   ...
   */
  public function get_contact_details($options = [])
  {
    $users_table                = $this->db->prefixTable('users');
  ...
  public function get_all_purchase_request($query)
  {
    $result = $this->db->query($query);
    if ($result) {
      return $result->getResultArray(); // Return results as an associative array
    } else {
  ...
  //Purchase request items that uses sku, title etc
  ...
  function get_project_po_total($project_id)
  {
    return $this->db->select_sum('total')
      ->where('project_id', $project_id)
      ->get(db_prefix() . 'purchase_orders')
      ->row()->total ?? 0;
  }
}
