...
$result = data_tables_purchase($aColumns, $sIndexColumn, $sTable, $join, $where, [db_prefix() . 'pur_invoices.id as id', 'invoice_number', 'currency',
], '', [], $dataPost);
...
foreach ($rResult as $aRow) {
 ...
 for ($i = 0; $i <aa count($aColumns); $i++) {
  ...
  if ($aColumns[$i] == 'invoice_number') {
   ...
   $delete = '<li role="presentation">' . modal_anchor(get_uri("purchase/delete_pur_invoice_modal"), "<i data-feather='x' class='icon-16'></i> " . app_lang('delete'), ["title" => app_lang('delete') . "?", "data-post-id" => $aRow['id'], "class" => "dropdown-item"]) . '</li>';
