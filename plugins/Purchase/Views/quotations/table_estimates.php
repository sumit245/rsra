...
$result = data_tables_purchase($aColumns, $sIndexColumn, $sTable, $join, $where, [
    db_prefix() . 'pur_estimates.id',
...
foreach ($rResult as $aRow) {
    ...
    log_message('error', 'Got row as' . print_r($aRow, true));
    if ($aRow['currency'] != '') {
    ...
    $amount = to_currency($aRow['total'], $base_currency);
    ...
    $row[] = $amount;
    $row[] = to_currency($aRow['total_tax'], $base_currency);
    ...
    $row[] = get_status_approve($aRow['status']);
    ...
    if ($dataPost['user_type'] == 'vendor') {
        if ($aRow['status'] != 2) {
            $delete = '<li role="presentation">' . modal_anchor(get_uri("purchase/delete_estimate_modal"), "<i data-feather='x' class='icon-16'></i> " . app_lang('delete'), ["title" => app_lang('delete') . "?", "data-post-id" => $aRow['id'], "class" => "dropdown-item"]) . '</li>';
        }
    } else {
        $delete = '<li role="presentation">' . modal_anchor(get_uri("purchase/delete_estimate_modal"), "<i data-feather='x' class='icon-16'></i> " . app_lang('delete'), ["title" => app_lang('delete') . "?", "data-post-id" => $aRow['id'], "class" => "dropdown-item"]) . '</li>';
    }
