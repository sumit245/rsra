<?php echo form_open(get_uri("purchase/send_po"), array("id" => "send-po-form", "class" => "general-form", "role" => "form")); ?>
<div id="send_po-dropzone" class="post-dropzone">
    <div class="modal-body clearfix">
        <div class="container-fluid">
            <input type="hidden" name="id" value="<?php echo html_entity_decode($po_info->id); ?>" />
            ...
            <div class="form-group">
                <div class="row">
                    <label for="po_bcc" class=" col-md-3">BCC</label>
                    <div class="col-md-9">
                        <?php
                        echo form_input(array(
                            "id" => "po_bcc",
            ...
            <div class="form-group ml15">
                <i data-feather="check-circle" class='icon-16 text-success' ></i> <?php echo app_lang('attached') . ' ' . anchor(get_uri("purchase/purorder_pdf/" . $po_info->id), $po_info->pur_order_number . ".pdf", array("target" => "_blank")); ?> 
            </div>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        var dropzone = attachDropzoneWithForm("#send_po-dropzone", uploadUrl, validationUri);

        $('#send-po-form .select2').select2();
        $("#send-po-form").appForm({
        ...
        $("#contact_id").select2().on("change", function () {
            ...
            if (contact_id) {
                ...
                appLoader.show();
                $.ajax({
                    url: "<?php echo get_uri('purchase/get_send_po_template/' . $po_info->id) ?>" + "/" + contact_id + "/json",
                    dataType: "json",
