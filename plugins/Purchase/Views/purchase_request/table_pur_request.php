...
//       $columnKey = end($columnKey); // Strip prefix
//       $_data = $aRow[$columnKey] ?? ''; // Fallback to empty if not found
...
//          if ($aRow['status'] == 2) {
//             $share_request = '<li role="presentation">' . modal_anchor(get_uri("purchase/share_request_modal/" . $aRow['id']), "<i data-feather='share-2' class='icon-16'></i> " . app_lang('share_pur_request'), ["title" => app_lang('share_request'), "data-post-id" => $aRow['id'], "class" => "dropdown-item"]) . '</li>';
//          }
...
//          $delete = '';
//          if (is_admin()) {
//             $delete = '<li role="presentation">' . modal_anchor(get_uri("purchase/delete_pur_request_modal"), "<i data-feather='x' class='icon-16'></i> " . app_lang('delete'), ["title" => app_lang('delete') . "?", "data-post-id" => $aRow['id'], "class" => "dropdown-item"]) . '</li>';
//          }
...
//          // Append hidden export-only div
//          $_data .= '<div class="export-items d-none">' . htmlentities(json_encode($aRow['items'])) . '</div>';
//       }
//       $row[] = $_data;
//    }
...
foreach ($rResult as $aRow) {
   ...
   foreach ($items as $item) {
      ...
      for ($i = 0; $i < count($aColumns); $i++) {
         ...
         if ($aColumns[$i] == db_prefix() . 'pur_request.status') {
            $row[] = $item['title'] ?? '';
            $row[] = $item['sku_code'] ?? '';
            $row[] = $item['sku_name'] ?? '';
         }
         if ($aColumns[$i] == 'request_date') {
            ...
            //    $approve_status .= '<ul class="dropdown-menu dropdown-menu-right" aria-labelledby="tablePurOderStatus-' . $aRow['id'] . '">';
            //    if ($aRow['status'] == 1) {
            //       $approve_status .= '<li><a href="#" onclick="change_pr_approve_status(2,' . $aRow['id'] . '); return false;">' . _l('purchase_approved') . '</a></li>';
            //       $approve_status .= '<li><a href="#" onclick="change_pr_approve_status(3,' . $aRow['id'] . '); return false;">' . _l('pur_rejected') . '</a></li>';
            //    } elseif ($aRow['status'] == 2) {
            //       $approve_status .= '<li><a href="#" onclick="change_pr_approve_status(1,' . $aRow['id'] . '); return false;">' . _l('purchase_draft') . '</a></li>';
            //       $approve_status .= '<li><a href="#" onclick="change_pr_approve_status(3,' . $aRow['id'] . '); return false;">' . _l('pur_rejected') . '</a></li>';
            //    } elseif ($aRow['status'] == 3) {
            //       $approve_status .= '<li><a href="#" onclick="change_pr_approve_status(1,' . $aRow['id'] . '); return false;">' . _l('purchase_draft') . '</a></li>';
            //       $approve_status .= '<li><a href="#" onclick="change_pr_approve_status(2,' . $aRow['id'] . '); return false;">' . _l('purchase_approved') . '</a></li>';
            ...
            $share_request = '';
            if ($aRow['status'] == 2) {
               $share_request = '<li role="presentation">' .
                  modal_anchor(
                     get_uri("purchase/share_request_modal/" . $aRow['id']),
                     "<i data-feather='share-2' class='icon-16'></i> " . app_lang('share_pur_request'),
                     ["title" => app_lang('share_request'), "data-post-id" => $aRow['id'], "class" => "dropdown-item"]
                  ) .
                  '</li>';
            }
            ...
            if (is_admin()) {
               $delete = '<li role="presentation">' .
                  modal_anchor(
                     get_uri("purchase/delete_pur_request_modal"),
                     "<i data-feather='x' class='icon-16'></i> " . app_lang('delete'),
                     ["title" => app_lang('delete') . "?", "data-post-id" => $aRow['id'], "class" => "dropdown-item"]
                  ) .
                  '</li>';
            }
