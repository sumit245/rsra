...
<div class="form-group  select-placeholder">
	<select name="<?php echo html_entity_decode($select_name) ?>" class="select2 <?php if ($ajaxItems == true) {
																						echo ' ajax-search';
																					} ?>" data-width="100%" id="<?php echo html_entity_decode($id_name) ?>" <?php if (isset($multiple)) { ?> multiple="<?php echo html_entity_decode($multiple) ?>" <?php } ?> placeholder="<?php if (isset($data_none_selected_text)) {
																																																																															echo _l($data_none_selected_text);
