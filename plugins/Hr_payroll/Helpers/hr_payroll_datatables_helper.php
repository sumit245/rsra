...
use Hr_payroll\Config\Hr_payroll;
...
if (!function_exists('data_tables_init1')) {
			...
			if (substr_count($column, '.') == 1 && strpos($column, ' as ') === false) {
				...
				if (isset($_column[1])) {
					if (startsWith1($_column[0], db_prefix())) {
						$_prefix = $Hr_payroll_model->prefixed_table_fields_wildcard($_column[0], $_column[0], $_column[1]);
	...
	";
		...
		$rResult = $Hr_payroll_model->hr_payroll_run_query($sQuery);
	...
	SELECT FOUND_ROWS()
	';
		$_query         = $Hr_payroll_model->hr_payroll_run_query($sQuery);
