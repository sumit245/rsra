<script>
	...
	function attendance_filter(invoker){
		...
		$.post("<?php echo get_uri("hr_payroll/attendance_filter") ?>", data).done(function(response) {
	...
	function attendance_calculation(event) {
		...
		$.post("<?php echo get_uri("hr_payroll/attendance_calculation") ?>", data).done(function(response) {
			response = JSON.parse(response);
			
			$(event).removeAttr('disabled')
			appAlert.success("<?php echo app_lang('updated_successfully') ; ?>");
