<script >
	(function(){
		"use strict";
		AppHelper.settings.disableKeyboardShortcuts = true;
		...
		var url_manage = "<?php echo get_uri("hr_payroll/payslip_manage") ?>";
		var options = {
			...
			functionButton: '<?php echo   html_entity_decode($permission_actions); ?>',
	...
	$(".luckysheet_info_detail_save").off('click').on('click', function(e){   
		...
		var image_flag = "false";
			...
			var rawCelldata = luckysheet.transToCellData(rawData[currentSheet ].data); 
			...
			$.ajax({
				...
				url: "<?php echo get_uri("hr_payroll/view_payslip_detail") ?>",
				type: 'POST',
				data: formData,
				contentType: false,
	...
	$(".luckysheet_info_detail_payslip_close").off('click').on('click', function(e){   
    	...
    	$.ajax({
    		...
    		url: "<?php echo get_uri("hr_payroll/payslip_closing") ?>",
    		type: 'POST',
    		data: formData,
    		contentType: false,
    		success: function (response, status, xhr) {
	...
	$(".luckysheet_info_detail_exports").off('click').on('click', function(e){   
		...
		var image_flag = "false";
			...
			$.ajax({ 
				...
				url: "<?php echo get_uri("hr_payroll/create_payslip_file") ?>", 
				method: 'post', 
				data: formData, 
				contentType: false, 
				processData: false
