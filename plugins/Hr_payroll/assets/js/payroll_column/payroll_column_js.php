<script>  
	...
	function new_column_type(){
		...
		$.post("<?php echo get_uri("hr_payroll/get_payroll_column_method_html_add") ?>").done(function(response) {
			response = JSON.parse(response);

			$('#add_payroll_column input[name="order_display"]').val(response.order_display);
			/*taking method*/
			$("select[id='taking_method']").html('');
			$("select[id='taking_method']").append(response.method_option);

			$("#add_payroll_column select[name='taking_method']").select2('destroy');
			$("#add_payroll_column select[name='taking_method']").select2();
	...
	function edit_column_type(invoker,id){
		...
		requestGetJSON('get_payroll_column/' + id).done(function (response) {
			...
			$("#add_payroll_column select[name='taking_method']").append(response.method_option.method_option);
			...
			$("#add_payroll_column select[name='function_name']").append(response.function_name.method_option);

			$("#add_payroll_column select[name='function_name']").select2('destroy');
			$("#add_payroll_column select[name='function_name']").select2();
	...
	$('select[name="taking_method"]').on('change', function(event) {
		...
		if(taking_method_value == 'system'){
			...
			$.post("<?php echo get_uri("hr_payroll/get_payroll_column_function_name_html") ?>").done(function(response){
				response = JSON.parse(response); 
				
				/*function name*/
				$("select[name='function_name']").html('');
				$("select[name='function_name']").append(response.method_option);
				$("#add_payroll_column select[name='function_name']").select2('destroy');
				$("#add_payroll_column select[name='function_name']").select2();
