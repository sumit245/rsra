<script>
	...
	var commodity_fill = new Handsontable(hotElement1, {
		...
		colHeaders: [
		"<?php echo app_lang('staffid') ?>",
		"<?php echo app_lang('hr_code') ?>",
		"<?php echo app_lang('staff_name') ?>",
		"<?php echo app_lang('job_position') ?>",
		"<?php echo app_lang('department') ?>",
		"<?php echo app_lang('hr_bonus_kpi') ?>",
		],
	...
	function bonus_filter(invoker){
		...
		$.post("<?php echo get_uri("hr_payroll/bonus_kpi_filter") ?>", data).done(function(response) {
	...
	function department_change(invoker){
		...
		$.post("<?php echo get_uri("hr_payroll/get_jobposition_fill_data") ?>",data_select).done(function(response){
			response = JSON.parse(response);
