<script>
	...
	var purchase = new Handsontable(hotElement1, {
		...
		function employees_filter (invoker){
			...
			$.post("<?php echo get_uri("hr_payroll/employees_filter") ?>", data).done(function(response) {
		...
		$('.hrp_employees_copy').on('click', function() {
			...
			$.post("<?php echo get_uri("hr_payroll/employees_copy") ?>", data).done(function(response) {
				response = JSON.parse(response);
