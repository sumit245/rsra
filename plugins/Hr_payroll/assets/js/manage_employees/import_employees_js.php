<script>
	...
	function uploadfilecsv(event){
		...
		if(($("#file_csv").val() != '') && ($("#file_csv").val().split('.').pop() == 'xlsx')){
			...
			$.ajax({ 
				...
				url: "<?php echo get_uri("hr_payroll/import_employees_excel") ?>",
				method: 'post', 
				data: formData, 
				contentType: false, 
				processData: false
				...
				{
					$( "#file_upload_response" ).append( '<a href="'+response.site_url+'\\'+response.filename+'" class="btn btn-warning text-white"  ><?php echo app_lang('hr_download_file_error') ?></a>' );
				}
	...
	function dowload_contract_excel(){
		...
		$.ajax({ 
			...
			url: "<?php echo get_uri("hr_payroll/create_employees_sample_file") ?>", 
			method: 'post', 
			data: formData, 
			contentType: false, 
			processData: false
			...
			if(response.success == true){
				...
				appAlert.success("<?php echo _l("create_attendance_file_success") ?>");
				...
				appAlert.warning("<?php echo _l("create_attendance_file_false") ?>");
			}
		});
	}
