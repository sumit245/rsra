...
class Hr_payroll_model extends Crud_model {
	 ...
	 * @return [type]               
	 */
	public function hr_payroll_run_query($query_string)
	{
	...
	public function get_table_data($table, $dataPost, $params = [])
	{
		...
		$path = HR_PAYROLL_VIEWPATH . 'admin/tables/' . $table . EXT;
		...
		if (!file_exists($path)) {
			...
			$myPrefixedPath = HR_PAYROLL_VIEWPATH . 'admin/tables/my_' . $table . EXT;
	...
	public function get_income_tax_rate($id = false){
		if (is_numeric($id)) {
			$builder = $this->db->table(get_db_prefix().'hrp_income_tax_rates');
			$builder->where('id', $id);
			return $builder->get()->getRow();
		}

		if ($id == false) {
			return $this->db->query('select * from '.get_db_prefix().'hrp_income_tax_rates')->getResultArray();
		}
	...
	public function update_income_tax_rates($data)
	{
		...
		$builder = $this->db->table(get_db_prefix().'hrp_income_tax_rates');
		$builder->where('id NOT IN ('.$row['delete'] .') ');
	...
	public function get_income_tax_rebates($id = false){
		if (is_numeric($id)) {
			$builder = $this->db->table(get_db_prefix().'hrp_income_tax_rebates');
			$builder->where('id', $id);
		...
		if ($id == false) {
			return $this->db->query('select * from '.get_db_prefix().'hrp_income_tax_rebates')->getResultArray();
		}
	...
	public function get_earnings_list($id = false){
		if (is_numeric($id)) {
			$builder = $this->db->table(get_db_prefix().'hrp_earnings_list');
			$builder->where('id', $id);
		...
		if ($id == false) {
			return $this->db->query('select * from '.get_db_prefix().'hrp_earnings_list')->getResultArray();
		}
	...
	public function get_salary_deductions_list($id = false){
		if (is_numeric($id)) {
			$builder = $this->db->table(get_db_prefix().'hrp_salary_deductions_list');
			$builder->where('id', $id);
		...
		if ($id == false) {
			return $this->db->query('select * from '.get_db_prefix().'hrp_salary_deductions_list')->getResultArray();
		}
	...
	public function get_insurance_list($id = false){
		if (is_numeric($id)) {
			$builder = $this->db->table(get_db_prefix().'hrp_insurance_list');
			$builder->where('id', $id);
		...
		if ($id == false) {
			return $this->db->query('select * from '.get_db_prefix().'hrp_insurance_list')->getResultArray();
		}
	...
	public function get_company_contributions_list($id = false){
		if (is_numeric($id)) {
			...
			return $this->db->get(get_db_prefix() . 'hrp_company_contributions_list')->getRow();
		}

		if ($id == false) {
			return $this->db->query('select * from '.get_db_prefix().'hrp_company_contributions_list')->getResultArray();
		}
	...
	public function update_company_contributions_list($data)
	{
		...
		$this->db->delete(get_db_prefix().'hrp_company_contributions_list');
		if($this->db->affected_rows() > 0){
			$affectedRows++;
		}

		if(count($row['insert']) != 0){
			$affected_rows = $this->db->insertBatch(get_db_prefix().'hrp_company_contributions_list', $row['insert']);
			if($affected_rows > 0){
				$affectedRows++;
			}
		}
		if(count($row['update']) != 0){
			$affected_rows = $this->db->updateBatch(get_db_prefix().'hrp_company_contributions_list', $row['update'], 'id');
			if($affected_rows > 0){
				$affectedRows++;
			}
		}
	...
	public function update_data_integration($data)
	{
		...
		$builder->where('setting_name', 'integrated_hrprofile');
		$builder->orWhere('setting_name', 'integrated_timesheets');
		$builder->orWhere('setting_name', 'integrated_commissions');
		$affectedrows = $builder->update($data_integration); 
		if($affectedrows > 0){
			$affected_rows++;
		}
		...
		if(count($data) > 0){
			foreach ($data as $key => $value) {
				...
				switch ($key) {
					...
					$affectedrows = $builder->update(['setting_value' => implode(',', $value)]); 
					if($affectedrows > 0){
						$affected_rows++;
					}
					break;
					...
					$builder = $this->db->table(get_db_prefix().'settings');
					$builder->where('setting_name', $key);
					$affectedrows = $builder->update(['setting_value' => $value]); 
					if($affectedrows > 0){
						$affected_rows++;
					}
					...
					default:
					$builder = $this->db->table(get_db_prefix().'settings');
					$builder->where('setting_name', $value);
					$affectedrows = $builder->update(['setting_value' => 1]); 
					if($affectedrows > 0){
						$affected_rows++;
					}
	...
	public function hr_records_get_earnings_list($id = false){
		if (is_numeric($id)) {
			$builder = $this->db->table(get_db_prefix().'hrp_earnings_list_hr_records');
			$builder->where('id', $id);
		...
		if ($id == false) {
			return $this->db->query('select * from '.get_db_prefix().'hrp_earnings_list_hr_records')->getResultArray();
		}
	...
	public function earnings_list_synchronization($data)
	{
		...
		if(hr_payroll_get_status_modules('Hr_profile') && get_setting('integrated_hrprofile') == 1){
			...
			$salary_types = $Hr_profile_model->get_salary_form();
			$allowance_types = $Hr_profile_model->get_allowance_type();
	...
	{	
		$builder = $this->db->table(get_db_prefix().'hrp_employees_header');
		$builder->where('rel_type', $rel_type);
		$builder->orderBy('header_oder', 'asc');
	...
	{
		$builder = $this->db->table(get_db_prefix().'hrp_employees_value');
		$builder->where('rel_type', $rel_type);
		$builder->orderBy('header_id', 'asc');
		return $builder->get()->getResultArray();
	}
	...
	public function get_employees_data1()
	{
		...
		$header_value[]='staff_id';
			...
			//get body
		$sql_query = "SELECT  staff_id, header_code, value, v.rel_type, v.header_id  FROM ".get_db_prefix()."hrp_employees_value as v
		left join ".get_db_prefix()."hrp_employees_header as h on h.id = v.header_id
		where v.rel_type = '".$rel_type."'
		order by staff_id, header_oder
		";
		$employees_value = $this->db->query($sql_query)->getResultArray();
		$body_temp=[];
	...
	public function get_employees_data($month, $rel_type ='', $where='')
	{
		...
		$builder->where('rel_type', $rel_type);
		$builder->where("date_format(month, '%Y-%m-%d') = '".$month."'");
	...
	{	
		...
		$probationary_date[] = app_lang('probationary_effective');
		$probationary_date[] = app_lang('probationary_expiration');
		$primary_date[] = app_lang('primary_effective');
		$primary_date[] = app_lang('primary_expiration');
	...
	public function get_list_staff_contract($month)
	{
		...
		$sql_temp = "select * FROM ".get_db_prefix()."hr_staff_contract 
		left join ".get_db_prefix()."hr_staff_contract_detail on ".get_db_prefix()."hr_staff_contract_detail.staff_contract_id = ".get_db_prefix()."hr_staff_contract.id_contract
		where ".get_db_prefix()."hr_staff_contract.id_contract IN (
		SELECT id_contract FROM ".get_db_prefix()."hr_staff_contract
		where ".get_db_prefix()."hr_staff_contract.contract_status = 'valid' AND date_format(start_valid, '%Y-%m-%d') >= '".$month."'

	)";

	$sql = "SELECT *  FROM ".get_db_prefix()."hr_staff_contract as ct
	left join ".get_db_prefix()."hr_staff_contract_detail on ".get_db_prefix()."hr_staff_contract_detail.staff_contract_id = ct.id_contract
	where (
	select count(*) from ".get_db_prefix()."hr_staff_contract as f
	...
	$staff_contracts = $this->db->query($sql)->getResultArray();
 ...
 */
	...
	public function employees_synchronization($data)
	{
		...
		if($data['department_employees_filter'] == '' && $data['staff_employees_filter'] == '' && $data['role_employees_filter'] == ''){
			...
			$builder = $this->db->table(get_db_prefix().'hrp_employees_value');
			$builder->where('id NOT IN ('.$row['delete'] .') and rel_type = "'.$rel_type.'" AND date_format(month,"%Y-%m-%d") = "'.$employees_month.'"');
			$affected_rows = $builder->delete();
			if($affected_rows > 0){
				$affectedRows++;
			}
	...
	public function get_hrp_attendance($month, $where='')
	{
		...
		$builder = $this->db->table(get_db_prefix().'hrp_employees_timesheets');
		if($where != ''){
			$builder->where($where);
		}
		$builder->where('rel_type', $rel_type);
		$builder->where("date_format(month, '%Y-%m-%d') = '".$month."'");
		$builder->orderBy('staff_id', 'asc');
		$employees_timesheets = $builder->get()->getResultArray();
	...
	public function get_day_header_in_month($month, $rel_type='')
	{
		...
		for ($d = 1; $d <= $total_day_in_month; $d++) {
			...
			switch($day){
				...
				$days_header_name[] = app_lang('sunday').' '. $d;
				break;
				case 1:
				$days_header['day_'.$d] = 0;
				$days_header_name[] = app_lang('monday').' '. $d;

				break;
				case 2:
				$days_header['day_'.$d] = 0;
				$days_header_name[] = app_lang('tuesday').' '. $d;

				break;
				case 3:
				$days_header['day_'.$d] = 0;
				$days_header_name[] = app_lang('wednesday').' '. $d;

				break;
				case 4:
				$days_header['day_'.$d] = 0;
				$days_header_name[] = app_lang('thursday').' '. $d;

				break;
				case 5:
				$days_header['day_'.$d] = 0;
				$days_header_name[] = app_lang('friday').' '. $d;

				break;
				case 6:
				$days_header['day_'.$d] = 0;
				$days_header_name[] = app_lang('saturday').' '. $d;
				break;
	...
	{	
		...
		$days_header_in_month = $this->get_day_header_in_month($attendance_month);
		...
		if($data['department_attendance_filter'] == '' && $data['staff_attendance_filter'] == '' && $data['role_attendance_filter'] == ''){
			...
			$builder = $this->db->table(get_db_prefix().'hrp_employees_timesheets');
			$builder->where('id NOT IN ('.$row['delete'] .') and rel_type = "'.$rel_type.'" AND date_format(month,"%Y-%m-%d") = "'.$attendance_month.'"');
			$affected_rows = $builder->delete();
			if($affected_rows > 0){
				$affectedRows++;
			}
	...
	public function synchronization_attendance($data)
	{
		...
		if(count($row['insert']) != 0){
			$affected_rows = $builder->insertBatch(get_db_prefix().'hrp_employees_timesheets', $row['insert']);
			if($affected_rows > 0){
				$affectedRows++;
			}
		}
		if(count($row['update']) != 0){
			$affected_rows = $builder->updateBatch(get_db_prefix().'hrp_employees_timesheets', $row['update'], 'id');
			if($affected_rows > 0){
				$affectedRows++;
			}
		}
	...
	public function hrp_get_timesheets_data($month, $rel_type)
	{
		...
		$sql_where_1 = "SELECT staff_id, type, sum(value) as total_time FROM ".get_db_prefix()."timesheets_timesheet
		where date_format(date_work,'%Y-%m') = '".$y_month."'
		group by staff_id, type
		;";

		$sql_where = "SELECT *, date_work as date_work_before, date_format(date_work, '%d') as date_work FROM ".get_db_prefix()."timesheets_timesheet
		where date_format(date_work,'%Y-%m') = '".$y_month."'
		;";
		...
		$timesheets = $this->db->query($sql_where)->getResultArray();
	...
	public function timesheet_get_shifts($month)
	{
		...
		$work_shifts = $this->db->get(get_db_prefix() . 'work_shift')->getResultArray();
		...
		foreach ($work_shifts as $value) {
			if($value['type_shiftwork'] == 'by_absolute_time'){
				...
				FROM ".get_db_prefix()."work_shift as ws
				left join ".get_db_prefix()."work_shift_detail as wsd on ws.id = wsd.work_shift_id
				left join ".get_db_prefix()."shift_type as st on wsd.shift_id = st.id
				where ws.id = ".$value['id']."
				";

				$shift_details = $this->db->query($sql_query)->getResultArray();
				...
				FROM ".get_db_prefix()."work_shift as ws
				left join ".get_db_prefix()."work_shift_detail_number_day as wsd on ws.id = wsd.work_shift_id
				left join ".get_db_prefix()."shift_type as st on wsd.shift_id = st.id
				where ws.id = ".$value['id']."
				";
				$shift_details = $this->db->query($sql_query)->getResultArray();
	...
	public function attendance_calculation($data)
	{
		...
		if(count($employees_timesheets) > 0){
			$builder = $this->db->table(get_db_prefix().'hrp_employees_timesheets');
			$builder->updateBatch($employees_timesheets, 'id');
		}
		return true;
	}
	...
	{	
		$builder = $this->db->table(get_db_prefix()."hrp_salary_deductions");
		...
		$builder->where("date_format(month, '%Y-%m-%d') = '".$month."'");
		$builder->orderBy('staff_id', 'asc');
		$deductions = $builder->get()->getResultArray();
	...
	public function deductions_update($data)
	{
		...
		if($data['department_deductions_filter'] == '' && $data['staff_deductions_filter'] == '' && $data['role_deductions_filter'] == ''){
			...
			$builder->where('id NOT IN ('.$row['delete'] .')  AND date_format(month,"%Y-%m-%d") = "'.$deductions_month.'"');
			$affected_rows = $builder->delete();
			if($affected_rows > 0){
				$affectedRows++;
			}
	...
	public function get_commissions_data($month, $where ='')
	{
		...
		$builder = $this->db->table(get_db_prefix()."hrp_commissions");
		if($where != ''){
			$builder->where($where);
		}
		$builder->where('rel_type', $rel_type);
	...
	public function commissions_update($data)
	{
		...
		if($data['department_commissions_filter'] == '' && $data['staff_commissions_filter'] == '' && $data['role_commissions_filter'] == ''){
			...
			$builder->where('id NOT IN ('.$row['delete'] .')  AND date_format(month,"%Y-%m-%d") = "'.$commissions_month.'" AND rel_type = "'.$rel_type.'"');
			$affected_rows = $builder->delete();
			if($affected_rows > 0){
				$affectedRows++;
			}
	...
	{
		$month = date('Y-m', strtotime($month));
		$sql_where ="SELECT c.staffid, sum(crd.amount_paid) as commission_amount FROM ".get_db_prefix()."commission_receipt as cr
		left join ".get_db_prefix()."commission_receipt_detail as crd on cr.id = crd.receipt_id
		left join ".get_db_prefix()."commission as c on crd.commission_id = c.id
		where date_format(cr.date, '%Y-%m') = '".$month."' AND c.is_client = '0'
		group by c.staffid";

		$commissions = $this->db->query($sql_where)->getResultArray();
	...
	public function get_income_tax_data($month)
	{
		...
		$builder = $this->db->table(get_db_prefix().'hrp_income_taxs');
		$builder->where("date_format(month, '%Y-%m-%d') = '".$month."'");
		$builder->orderBy('staff_id', 'asc');
		$income_taxs = $builder->get()->getResultArray();
		return $income_taxs;
	}
	...
	public function get_total_income_tax_in_year($month)
	{
		...
		$builder->select('staff_id, sum(income_tax) as tax_for_year');
		$builder->where("date_format(month, '%Y') = '".$month."'");
	...
	{	
		$builder = $this->db->table(get_db_prefix()."hrp_staff_insurances");
		...
		$builder->where("date_format(month, '%Y-%m-%d') = '".$month."'");
		$builder->orderBy('staff_id', 'asc');
		$insurances = $builder->get()->getResultArray();
	...
	public function insurances_update($data)
	{
		...
		if($data['department_insurances_filter'] == '' && $data['staff_insurances_filter'] == '' && $data['role_insurances_filter'] == ''){
			...
			$builder->where('id NOT IN ('.$row['delete'] .')  AND date_format(month,"%Y-%m-%d") = "'.$insurances_month.'"');
			$affected_rows = $builder->delete();
			if($affected_rows > 0){
				$affectedRows++;
			}
	...
	public function get_hrp_payroll_columns($id = false){
		if (is_numeric($id)) {
			$builder = $this->db->table(get_db_prefix().'hrp_payroll_columns');
			$builder->where('id', $id);
			return $builder->get()->getRow();
		}
		if ($id == false) {
			return $this->db->query('select * from ' . db_prefix() . 'hrp_payroll_columns order by order_display asc')->getResultArray();
		}
	}
	...
	public function get_list_payroll_column_method($data)
	{
		...
		if(isset($data['taking_method']) && $data['taking_method'] != ''){
			...
			$list_column_method=[];
			$list_column_method[] = ['name' => 'system', 'lable' => app_lang('system_method')];
			$list_column_method[] = ['name' => 'caculator', 'lable' => app_lang('caculator_method')];
			$list_column_method[] = ['name' => 'constant', 'lable' => app_lang('constant_method')];
			...
			foreach ($list_column_method as $method) {
				...
				if($method['name'] == $data['taking_method']){           
					...
					$method_option_selected = $method['name'];
				}
			...
			$method_option .= '<option value="system">' . app_lang('system_method'). '</option>';
			$method_option .= '<option value="caculator">' . app_lang('caculator_method'). '</option>';
			$method_option .= '<option value="constant">' . app_lang('constant_method'). '</option>';

		}
		
		$data_return =[];
		$data_return['method_option'] = $method_option;
		$data_return['method_option_selected'] = $method_option_selected;
	...
	public function get_list_payroll_column_function_name($data)
	{
			...
			foreach ($list_column_method as $method) {
				if(!in_array($method['function_name'], $payroll_system_columns) && !in_array($method['function_name'], $hrp_payroll_columns) || $method['function_name'] == $data['function_name']){
					...
					if($method['function_name'] == $data['function_name']){           
						$select .= 'selected="selected"';
						$function_name_selected = $method['function_name'];
					}
		...
		$data_return =[];
		$data_return['method_option'] = $method_option;
		$data_return['function_name_selected'] = $function_name_selected;
	...
	public function add_payroll_column($data){
		...
		$data['staff_id_created'] = get_staff_user_id1();
		$data['date_created'] = to_sql_date1(get_my_local_time("Y-m-d H:i:s"), true);
	...
	public function update_payroll_column($data, $id){
		$hrp_payroll_column = $this->get_hrp_payroll_columns($id);
	...
	public function get_hrp_payslip_templates($id = false){
		if (is_numeric($id)) {
			$builder = $this->db->table(get_db_prefix()."hrp_payslip_templates");

			$builder->where('id', $id);
			return $builder->get()->getRow();
		}
		if ($id == false) {
			return $this->db->query('select * from ' . db_prefix() . 'hrp_payslip_templates order by id desc')->getResultArray();
		}
	}
	...
	public function get_hrp_payslip($id = false){
		if (is_numeric($id)) {
			$builder = $this->db->table(get_db_prefix()."hrp_payslips");

			$builder->where('id', $id);
			return $builder->get()->getRow();
		}
		if ($id == false) {
			return $this->db->query('select * from ' . db_prefix() . 'hrp_payslips order by id desc')->getResultArray();
		}
	}
	...
	public function delete_payslip($id)
	{
		...
		app_hooks()->do_action('before_payslip_deleted', $id);
		
		$affected_rows =0;

		$payslip = $this->get_hrp_payslip($id);
		if ($payslip && file_exists(HR_PAYROLL_PAYSLIP_FILE.$payslip->file_name)) {
	...
	{
		$builder = $this->db->table(get_db_prefix().'hrp_payslip_details');
		$builder->where('payslip_id', $payslip_id);
		$affected_rows = $builder->delete();
		if ($affected_rows > 0) {
			return true;
		}
	...
	public function add_payslip_templates_detail_first($id, $update= false, $old_column_formular=[])
	{
		...
		if($payslip_templates){
			...
			$sql_where = "SELECT id, column_key, function_name, taking_method FROM ".get_db_prefix()."hrp_payroll_columns where find_in_set(id, '".$payslip_templates->payslip_columns."') order by order_display";
			$payroll_columns = $this->db->query($sql_where)->getResultArray();
	...
	public function get_all_staff_departments()
	{
		...
		$staff=[];
		$teams = $this->db->query('select * from '.get_db_prefix().'team where deleted = 0 ')->getResultArray();
		foreach ($teams as $staff_department) {
		    $members = explode(",", $staff_department['members']);
		    foreach ($members as $key => $member) {
		    	if(strlen($member) > 0 && is_numeric($member)){
		    		if(isset($staff[$member])){
		    			$staff[$member] = $staff[$member].', '.$staff_department['title'];
		    		}else{
		    			$staff[$member] = $staff_department['title'];
		    		}
		    	}
		    }
	...
	{
		$builder = $this->db->table(get_db_prefix()."hrp_bonus_kpi");

		$builder->where('staffid', $staffid);
		$builder->where('month_bonus_kpi', $month);
	...
	public function get_bonus_kpi($month, $where='')
	{
		...
		$builder->select('*, staffid as staff_id');
		$builder->where('month_bonus_kpi', $month);

		$bonus_kpi = $builder->get()->getResultArray();
		return $bonus_kpi;
	}
	...
	public function get_staff_timekeeping_applicable_object($where = [])
	{
		...
		$builder->select('*,CONCAT(first_name," ",last_name) as full_name, '.get_db_prefix().'users.id as staffid');
		if($rel_type == 'hr_records'){
			$builder->join(get_db_prefix() . 'hr_job_position', db_prefix() . 'users.job_position = ' . db_prefix() . 'hr_job_position.position_id', 'left');
		}
	...
	public function add_bonus_kpi($data)
	{
		...
		foreach ($data_data_bonus_kpi as  $data_bonus_key => $data_bonus_value) {
			...
			if(($data_bonus_key+1)%6 == 0){
				...
				$builder = $this->db->table(get_db_prefix()."hrp_bonus_kpi");
				$builder->where('staffid', $arr_temp['staffid']);
				$builder->where('month_bonus_kpi', $month_add_update);

				$staff_point_in_month = $builder->get()->getRow();

				if($staff_point_in_month){
					//update
					$builder = $this->db->table(get_db_prefix()."hrp_bonus_kpi");
					
					$builder->where('id', $staff_point_in_month->id);
					$affected_rows = $builder->update($arr_temp);
					if ($affected_rows > 0) {
						$results_update = true;
					}
	...
	public function getStaff($id = '', $where = [])
	{
		...
		if (is_staff_logged_in() && $id != '' && $id == get_staff_user_id1()) {
			$select_str .= ',(SELECT COUNT(*) FROM ' . db_prefix() . 'notifications WHERE touserid=' . get_staff_user_id1() . ' and isread=0) as total_unread_notifications, (SELECT COUNT(*) FROM ' . db_prefix() . 'todos WHERE finished=0 AND staffid=' . get_staff_user_id1() . ') as total_unfinished_todos';
		}
		...
		if (is_numeric($id)) {
			...
			$staff = $this->db->get(get_db_prefix() . 'staff')->getRow();
	...
	public function payslip_template_get_staffid($department_ids, $role_ids, $staff_ids, $except_staff='')
	{
		...
		if(strlen($department_ids) > 0){
			$arrdepartment = $this->get_department_by_list_id($department_ids);
			$temp = '';

			foreach ($arrdepartment as $value) {
				if($value['members'] != '' && strlen($value['members']) > 0){
					if(strlen($value['members']) > 0){
						$temp = $temp.$value['members'].',';
					}
				}
			}
		...
		if( strlen($role_ids) > 0){
			$role_querystring = 'FIND_IN_SET(role_id, "'.$role_ids.'")';
		}

		if( strlen($except_staff) > 0){
			$except_staff_querystring = 'id NOT IN ('.$except_staff .')' ;
		}
	...
	{   	
		...
		$str_sql1 = 'id  IN (0) AND status = "active"';
		$str_bonus_kpi_sql1 = 'staffid  IN (0)';
		$str_user_sql1 = 'user_id  IN (0)';
		$str_sql = 'staff_id  IN (0)';
		$str_user_sql = 'id  IN (0)';
		$str_user_id_sql1 = 'user_id IN (0)';
		if($staffids != false){
			$str_sql1 = 'id  IN ('.$staffids .')  AND status = "active"';
			$str_bonus_kpi_sql1 = 'staffid  IN ('.$staffids .')';

			$str_user_sql1 = 'user_id  IN ('.$staffids .')';
			$str_sql = 'staff_id  IN ('.$staffids .')';
			$str_user_sql = 'id  IN ('.$staffids .')';

			$str_user_id_sql1 = 'user_id IN ('.$staffids .')';
		}
		...
		foreach ($staffs as $staff_key => $staff_value) {
			$staff_value['employee_name'] = $staff_value['first_name'].' '.$staff_value['last_name'];
			$staff_value['payment_run_date'] = to_sql_date1(get_my_local_time("Y-m-d"), true);
		...
		//get bonus kpi
		$bonus_kpi_data = $this->get_bonus_kpi($payslip_month, $str_bonus_kpi_sql1);
		...
		//get salary by task
		$salary_by_tasks = $this->get_tasks_timer_by_month($payslip_month, $str_user_sql, $str_user_sql1, $hr_profile_status, $str_user_id_sql1);
		...
		// set company logo
		$payslip_cell_data[] = $this->general_cell_data(1, 4, get_default_company_name() , $t='g', $f ='', false, false, true);
		$payslip_cell_data[] = $this->general_cell_data(2, 4, app_lang('payroll_in_month').$data['payslip_month'] , $t='g', $f ='', false, true, false);
		...
		if(count($staffs_id) > 0){
			...
			$payslip_cell_data[] = $this->general_cell_data(5, 4, app_lang('no_eligible_employee_was_found_for_this_payslip_template'), $t='g', $f ='', false, false, true);
		...
		$builder->insert([
			...
			'staff_id_created' => get_staff_user_id1(),
			'date_created' => to_sql_date1(get_my_local_time("Y-m-d H:i:s"), true),
	...
	{	
		...
		$affected_rows = $builder->update(['file_name' => $realpath_data]);
	...
	{	
		...
		if(count($payslip_detail) != 0){
			...
			$builder = $this->db->table(get_db_prefix().'hrp_payslip_details');
			$builder->where('payslip_id', $data['id']);
			$builder->where('month', $month);
			$builder->delete();

			//delete mass hrp_income_taxs before update
			$builder = $this->db->table(get_db_prefix().'hrp_income_taxs');
			$builder->where('staff_id IN ('.implode(",",$staff_ids) .') ');
			$builder->where('month', $month);
			$builder->delete();

			$builder = $this->db->table(get_db_prefix().'hrp_payslip_details');
			$affected_rows = $builder->insertBatch($payslip_detail);
			if($affected_rows > 0){
				$affectedRows++;
			}

			$builder = $this->db->table(get_db_prefix().'hrp_income_taxs');
			$affected_rows = $builder->insertBatch($income_taxs);
			if($affected_rows > 0){
				$affectedRows++;
			}
		}
	...
	{	
		...
		$builder->where('id', $id);
		$affected_rows = $builder->update(['payslip_status' => $status]);
		if($affected_rows > 0){
			return true;
		}
		return false;
	...
	public function get_payslip_detail($id = false)
	{
		...
		if ($id == false) {
			return $this->db->query('select * from '.get_db_prefix().'hrp_payslip_details')->getResultArray();
		}
	}
	...
	public function get_income_summary_report($sql_where='')
	{
		...
		$builder->select(get_db_prefix().'hrp_payslip_details.staff_id, pay_slip_number, employee_number, employee_name,  month, net_pay, ' . db_prefix() . 'hrp_payslips.payslip_status,'.get_db_prefix().'hrp_payslip_details.employee_number');
		$builder->join(get_db_prefix() . 'hrp_payslips', db_prefix() . 'hrp_payslip_details.payslip_id = ' . db_prefix() . 'hrp_payslips.id', 'left');
		if($sql_where != ''){
			$builder->where($sql_where);
		}
		$builder->orderBy('staff_id', 'desc');
		$payslip_details = $builder->get()->getResultArray(); 
	...
	{
		$builder = $this->db->table(get_db_prefix().'hrp_payslip_details');
		$builder->select(get_db_prefix().'hrp_payslip_details.staff_id, sum(total_insurance) as total_insurance');
		$builder->join(get_db_prefix() . 'hrp_payslips', db_prefix() . 'hrp_payslip_details.payslip_id = ' . db_prefix() . 'hrp_payslips.id', 'left');
		if($sql_where != ''){
			$builder->where($sql_where);
		}
		$builder->groupBy(get_db_prefix().'hrp_payslip_details.staff_id');
		$builder->orderBy(get_db_prefix().'hrp_payslip_details.staff_id', 'desc');
		$payslip_details = $builder->get()->getResultArray(); 
	...
	public function payslip_chart($filter_by_year = '', $staff_id='')
	{
		...
		$builder = $this->db->table(get_db_prefix().'hrp_payslip_details');
		$builder->select(get_db_prefix().'hrp_payslip_details.staff_id, pay_slip_number, employee_number, employee_name,  month, net_pay, ' . db_prefix() . 'hrp_payslips.payslip_status, total_insurance, income_tax_paye, total_deductions');
		$builder->join(get_db_prefix() . 'hrp_payslips', db_prefix() . 'hrp_payslip_details.payslip_id = ' . db_prefix() . 'hrp_payslips.id', 'left');
		$builder->where($filter_by_year.' AND '.get_db_prefix().'hrp_payslip_details.staff_id = '.$staff_id.' AND payslip_status = "payslip_closing"');
		$builder->orderBy('staff_id', 'desc');
		$payslip_details = $builder->get()->getResultArray(); 
		...
		for($_month = 1 ; $_month <= 12; $_month++){
			...
			if($_month == 5){
				$chart['categories'][] = app_lang('month_05');
			}else{
				$chart['categories'][] = app_lang('month_'.$_month);
			}
	...
	{	
		$builder = $this->db->table(get_db_prefix().'team');
		$builder->select('*, id as departmentid, title as name');
		$staff_departments = $builder->get()->getResultArray();

		//select payslip detail by month
		$builder = $this->db->table(get_db_prefix().'hrp_payslip_details');
		$builder->where('month >= ', $from_date);
		$builder->where('month <= ', $to_date);
		$builder->orderBy('staff_id', 'asc');
		$payslip_details = $builder->get()->getResultArray();
		...
		foreach ($staff_departments as $key => $staff_department) {
			...
			foreach ($members as $member) {
				if(strlen($member) > 0 && is_numeric($member)){
					...
					if(isset($staff_payslip[$member])){

						if(isset($data_result[$staff_department['departmentid']])){

							$data_result[$staff_department['departmentid']]['gross_pay'] += $staff_payslip[$member]['gross_pay'];
							$data_result[$staff_department['departmentid']]['total_insurance'] += $staff_payslip[$member]['total_insurance'];
							$data_result[$staff_department['departmentid']]['income_tax_paye'] += $staff_payslip[$member]['income_tax_paye'];
							$data_result[$staff_department['departmentid']]['total_deductions'] += $staff_payslip[$member]['total_deductions'];
							$data_result[$staff_department['departmentid']]['commission_amount'] += $staff_payslip[$member]['commission_amount'];
							$data_result[$staff_department['departmentid']]['bonus_kpi'] += $staff_payslip[$member]['bonus_kpi'];
							$data_result[$staff_department['departmentid']]['net_pay'] += $staff_payslip[$member]['net_pay'];
							$data_result[$staff_department['departmentid']]['total_cost'] += $staff_payslip[$member]['total_cost'];
						}else{
		...
		foreach ($payslip_columns as $payslip_column) {
			...
			switch ($payslip_column) {
				case 'gross_pay':
				$payslip_column_name = app_lang('ps_gross_pay');
				break;
				case 'total_insurance':
				$payslip_column_name = app_lang('ps_total_insurance');
				break;
				case 'income_tax_paye':
				$payslip_column_name = app_lang('ps_income_tax_paye');
				break;
				case 'total_deductions':
				$payslip_column_name = app_lang('ps_total_deductions');
				break;
				case 'commission_amount':
				$payslip_column_name = app_lang('ps_commission_amount');
				break;
				case 'bonus_kpi':
				$payslip_column_name = app_lang('ps_bonus_kpi');
				break;
				case 'net_pay':
				$payslip_column_name = app_lang('ps_net_pay');
				break;
				case 'total_cost':
				$payslip_column_name = app_lang('ps_total_cost');
				break;
		...
		if($staff_ids != false){
			...
			if(count($array_staff_ids) > 0){
				...
				if(isset($data['id']) && is_numeric($data['id'])){
					...
					$payslip_templates = $builder->get()->getResultArray();
				}else{
					//add payslip template
				...
				if(count($staff_has_template) > 0){
					$staff_str_query = ' id IN ('.implode(",",$staff_has_template) .') ';
					...
					foreach ($array_staff_has_template as $key => $value) {
						if(strlen($str_staff_has_template) > 0){
							$str_staff_has_template .= ', '. $value['first_name'].' '.$value['last_name'];
					...
					$str_staff_has_template .= app_lang('falls_within_other_the_payslip_template');
	...
	{	
		...
		$builder = $this->db->table(get_db_prefix().'hrp_payslips');
		$builder->where('payslip_month', $payslip_month);
		$builder->where('payslip_template_id', $payslip_template_id);
		$builder->where('payslip_status', 'payslip_closing');
		$payslip_closing = $builder->get()->getResultArray();
	...
	public function employees_copy($data)
	{
		...
		if(count($employees) > 0){
			...
			$builder->where('rel_type', $rel_type);
			$builder->where("date_format(month, '%Y-%m-%d') = '".date('Y-m-d', strtotime($data['month'].'-01'))."'");
			$affected_rows = $builder->delete();
			if($affected_rows > 0){
				$affectedRows++;
			}
			...
			$builder = $this->db->table(get_db_prefix().'hrp_employees_value');
			$affected_rows = $builder->insertBatch($employees);
			if($affected_rows > 0){
				$affectedRows++;
			}

			if($affectedRows > 0){
				$message = app_lang('hrp_added_successfully');
				$status = 'success';
			}else{
				$message = app_lang('hrp_add_failed');
				$status = 'warning';
			}
		}else{

			$message = app_lang('No_data_for_the_previous_month');
			$status = 'warning';
		}
	 ...
	 * @return [type]        
	 */
	public function get_tasks_timer_by_month($month, $staff_id, $str_id_sql1, $hr_profile_status, $str_user_id_sql1)
	{
		$month_temp = $month;
		$salary_task_timers=[];
		...
		//get_staff
		$builder = $this->db->table(get_db_prefix().'users');
		$builder->where($staff_id);
		$builder->orderBy('first_name', 'desc');
		$staffs = $builder->get()->getResultArray();

		$staff_data=[];
		foreach ($staffs as $key => $staff) {
			$staff_data[$staff['id']] = $staff;
		}

		/*get staff salary from job infor*/
		$builder = $this->db->table(get_db_prefix().'team_member_job_info');
		$builder->where($str_user_id_sql1);
		$team_member_job_infos = $builder->get()->getResultArray();

		$team_member_job_info_data=[];
		foreach ($team_member_job_infos as $key => $job_info) {
			$team_member_job_info_data[$job_info['user_id']] = $job_info;
		}
		...
		$sql_where="SELECT ".get_db_prefix()."project_time.user_id as staff_id, start_time, end_time, date_format(end_time, '%c') as months, TIMESTAMPDIFF(MINUTE, start_time, end_time) as total_time FROM ".get_db_prefix()."project_time 
		where date_format(end_time, '%c') = ".$month." AND ".$str_user_id_sql1."
		order by staff_id desc";

		$task_timers = $this->db->query($sql_where)->getResultArray();

		foreach ($task_timers as $key => $task_timer) {
			$task_timer_hourly_rate = 0;
			if(isset($team_member_job_info_data[$task_timer['staff_id']])){
				$task_timer_hourly_rate = (float)$team_member_job_info_data[$task_timer['staff_id']]['salary'];
			}

			if($task_timer_hourly_rate != 0){
				if(isset($salary_task_timers[$task_timer['staff_id']])){

					$salary_task_timers[$task_timer['staff_id']]['salary_from_tasks'] += (float)$task_timer_hourly_rate*((float)$task_timer['total_time']/60);
				...
				//get by contract
					...
					//probationary contract
						...
						if($flag_contract == false){
							...
							if(isset($team_member_job_info_data[$task_timer['staff_id']])){
								$hourly_rate = (float)$team_member_job_info_data[$task_timer['staff_id']]['salary'];
							}
						}
