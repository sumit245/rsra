	...
	<div class="modal-dialog modal-lg">
		<div class="modal-content">
			<div class="modal-header">
				...
				$staffid = '';
				if (isset($member)) {
					$title .= app_lang('hrp_update_permissions');
					$staffid = $member->staffid;
				} else {
					$title .= app_lang('hrp_add_permissions');
				}
				?>
				<h4 class="modal-title"><?php echo html_entity_decode($title); ?></h4>
			</div>
			<?php echo form_open(admin_url('hr_payroll/hr_payroll_update_permissions/' . $staffid), ['id' => 'update_permissions']); ?>
			<div class="modal-body">
				...
				<div class="table-responsive">
					...
					<div class="hide">
						<?php
						$isadmin = '';
						if (isset($member) && ($member->staffid == get_staff_user_id1() || is_admin($member->staffid))) {
							$isadmin = ' checked';
						}
						?>
					...
					?>
					<div class="class">
						<?php echo render_select('role', $roles_value, ['roleid', 'name'], 'staff_add_edit_role', $selected); ?>
					</div>
					...
					<div class="class					                 					                  <?php echo html_entity_decode($display_staff); ?>">
						<?php echo render_select('staff_id', $staffs, ['staffid', ['firstname', 'lastname']], 'hrp_staff_name', $staff_selected); ?>
					</div>
					...
					<table class="table table-bordered roles no-margin">
						...
						<tbody>
							...
							foreach (get_available_staff_permissions($funcData) as $feature => $permission) { ?>
								<tr data-name="<?php echo html_entity_decode($feature); ?>" class="<?php if (! in_array($feature, $hr_payroll_permissions)) {
																										echo "hide";
																									} ?>">
									<td>
			...
			<div class="modal-footer">
				<button type="button" class="btn btn-default close_btn" data-dismiss="modal"><?php echo app_lang('hrp_close'); ?></button>
