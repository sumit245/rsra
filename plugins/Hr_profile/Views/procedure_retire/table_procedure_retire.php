...
foreach ($rResult as $aRow) {
    ...
    $_data .= '<a href="' . admin_url('hr_profile/procedure_procedure_retire_details/' . $aRow['id']) . '">' . app_lang('hr_view') . '</a>';

    if(hr_has_permission('hr_profile_can_edit_setting') || is_admin()){ 
        $_data .= ' | <a href="#" onclick="edit_procedure_form_manage(this); return false;" data-id="'.$aRow['id'].'"  data-name_procedure_retire="'.$aRow['name_procedure_retire'].'"  data-department="'.$list_department.'" >' . app_lang('hr_edit') . '</a>';
    }

    if(hr_has_permission('hr_profile_can_delete_setting') || is_admin()){ 
        $_data .= ' | <a href="' . admin_url('hr_profile/delete_procedure_form_manage/' . $aRow['id']) . '"  class="text-danger _delete">' . app_lang('delete') . '</a>';
    }
    $_data .= '</div>';
