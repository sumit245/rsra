										...
										<div class="form-group">
											...
											<select name="training_type" class="select2 validate-hidden" id="training_type" data-width="100%" placeholder="<?php echo app_lang('dropdown_non_selected_tex'); ?>">
												...
												<?php foreach ($type_of_trainings as $key => $value) { ?>
													<option value="<?php echo html_entity_decode($value['id']) ?>" <?php if (isset($setting_training) && $setting_training->training_type == $value['id']) {
																														echo 'selected';
																													}; ?>><?php echo html_entity_decode($value['name'])  ?></option>
												<?php } ?>
											</select>
										</div>
									</div>
