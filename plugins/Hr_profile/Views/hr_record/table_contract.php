...
foreach ($rResult as $aRow) {
	...
	if (has_permission('hr_profile_contract', '', 'view') || is_admin()) {
		$subjectOutput .= '<a href="' . admin_url('hr_profile/contracts/' . $aRow['id'] ).'" onclick="init_hr_profile_contract(' . $aRow['id'] . ');return false;">' . app_lang('hr_view') .' </a>';
	}
	if (has_permission('hr_profile_contract', '', 'edit') || is_admin()) {
		$subjectOutput .= ' | <a href="' . admin_url('hr_profile/contract/' . $aRow['id']) . '">' . app_lang('hr_edit') . '</a>';
	}

	if (has_permission('hr_profile_contract', '', 'delete') || is_admin()) {
		$subjectOutput .= ' | <a href="' . admin_url('hr_profile/delete_contract/' . $aRow['id']) . '" class="text-danger _delete">' . app_lang('delete') . '</a>';
	}
	...
	if($aRow['contract_status'] == 'draft' ){
		$row[] = ' <span class="label label inline-block text-white bg-gray"> '.app_lang('draft').' </span>';
	}elseif($aRow['contract_status'] == 'valid'){
		$row[] = ' <span class="label label inline-block text-white bg-primary"> '.app_lang('valid').' </span>';
	}elseif($aRow['contract_status'] == 'invalid'){
		$row[] = ' <span class="label label inline-block text-white bg-danger"> '.app_lang('invalid__').' </span>';
	}elseif($aRow['contract_status'] == 'finish'){
		$row[] = ' <span class="label label inline-block text-white bg-success"> '.app_lang('finish').' </span>';
	}else{
		$row[] = '';
	}
