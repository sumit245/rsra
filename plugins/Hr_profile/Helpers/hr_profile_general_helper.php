  ...
  if ($_FILES) {
    ...
    if ($image_file_name) {
      if (! $this->check_profile_image_dimension($image_file_name)) {
        echo json_encode(["success" => false, 'message' => app_lang('profile_image_error_message')]);
        exit();
      }
      ...
      //delete old file
      delete_app_files(get_setting("profile_image_path"), [@unserialize($user_info->image)]);
...
if (! function_exists('hr_log_notification')) {
  ...
  function hr_log_notification($event, $options = [], $user_id = 0, $to_user_id = 0)
  {
