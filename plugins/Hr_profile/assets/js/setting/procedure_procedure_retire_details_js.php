<script>       
	...
	function add_procedure_retire(){
		...
		html += '<select name="people_handle_id" id="people_handle_id" class="select2 validate-hidden" data-none-selected-text="<?php echo _l('hr_people_handle_id'); ?>" data-live-search="true">';
		html += '<option value=""></option>';
		html += '<?php foreach ($staffs as $s) { ?><option value="<?php echo html_entity_decode($s['id']); ?>" ><?php echo get_staff_full_name1($s['id']); ?></option><?php } ?>';
		html += '</select></div>';
		html += '</div>';
		...
		html += '<button type="button" class="add_remove_action add_more_box btn btn-info text-white" onclick="add_option();"><span data-feather="plus-circle" class="icon-16"></span> </button>';
		html += '</div>';
		...
		$('#add_save').remove();
		$('._buttons').append('<button type="submit" class="btn btn-info pull-left mleft10 text-white"><span data-feather="check-circle" class="icon-16"></span>  <?php echo _l('submit'); ?></button>');
		$(".select2").select2();
	}
	...
	function add_option() {
		...
		html += '<button class="add_remove_action add_more_box text-white btn btn-danger" onclick="remove_question(this);"><span data-feather="x" class="icon-16"></span>  </button>';
		html += '</div>';

		html += '<div class="col-md-11">';
		html += '<input type="text" placeholder="<?php echo _l('hr_add_options'); ?>..." name="option_name[1]['+total+']" id="option_name[1]['+total+']" data-box-descriptionid="" class="my-1 form-control">';
		html += '</div>';
	...
	function edit_procedure_retire(el) {
		...
		$.post("<?php echo get_uri("hr_profile/edit_procedure_retire/") ?>"+id).done(function(response) {
			...
			$.each(response.option_name, function(e, v){
				...
				if(e == 1){
					...
					html += '<button type="button" class="add_remove_action add_more_box btn btn-info text-white" onclick="add_option_modal(this);"><span data-feather="plus-circle" class="icon-16"></span> </button>';
				...
				html += '<input type="text" placeholder="<?php echo _l('hr_add_options'); ?>..." name="option_name[1]['+edit_index+']" id="option_name[1]['+edit_index+']" data-box-descriptionid="" class=" form-control" value="'+v+'">';
				html += '</div>';
				html += '</div><br/><div class="clearfix"></div>';
				html += '</div>';
			...
			$('select[name="people_handle_id"]').val(response.people_handle_id).change();
			$(".select2").select2();
	...
	function add_option_modal() {
		...
		html += '<div class="col-md-11">';
		html += '<input type="text" placeholder="<?php echo _l('hr_add_options'); ?>..." name="option_name[1]['+detail_index+']" id="option_name[1]['+detail_index+']" data-box-descriptionid="" class="form-control">';
		html += '</div>';
		html += '</div><br/><div class="clearfix"></div>';
		html += '</div>';
