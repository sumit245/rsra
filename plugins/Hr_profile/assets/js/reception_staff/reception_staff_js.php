	<script>	
		...
		function active_asset(el){
			...
			$.post("<?php echo get_uri("hr_profile/change_status_allocation_asset") ?>", data).done(function(response){
		...
		function change_info_checklist(el){
			...
			$.post("<?php echo get_uri("hr_profile/change_status_checklist") ?>", data).done(function(response){
				response = JSON.parse(response);
				if(response.success == true) { 
					change_progress();
				}
		...
		function save_add_asset(){
			...
			$.post("<?php echo get_uri("hr_profile/add_new_asset/".$staff->id) ?>",data).done(function(response){
				response = JSON.parse(response);
				if(response.success == true) {
		...
		function delete_asset(el){
			...
			$.post("<?php echo get_uri("hr_profile/delete_asset/") ?>"+val+"<?php echo "/".$staff->id ?>").done(function(response){
				response = JSON.parse(response);
				if(response.success == true) {
					$('#asset_list').html(response.data);
						appAlert.success('<?php echo _l('hr_deleted'); ?>');
