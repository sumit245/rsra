<script>
	...
	function staff_chart_by_age(id, value, title_c){
		...
		Highcharts.chart(id, {
			...
			tooltip: {
				pointFormat: '<span style="color:{series.color}">'+<?php echo json_encode(_l('hr_qty')); ?>+'</span>: <b>{point.y}</b> <br/>',
				shared: true
			},
			legend: {
				enabled: false
    ...
    'use strict';
		...
		requestGetJSON('get_chart_senior_staff/' + sort_from+'/'+months_report).done(function (response) {
...
function report_by_staffs(id, value, title_c){
	...
	requestGetJSON('report_by_staffs').done(function (response) {
...
$(function(){
	...
	initDataTable(table_staff,"<?php echo get_uri("hr_profile/table_reception") ?>",  [0], [0], '');
