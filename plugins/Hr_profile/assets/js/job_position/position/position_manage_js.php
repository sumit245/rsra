<script type="text/javascript">
	(function(){
		...
		$("body").on('click', '.tagit-close', function() {
			...
			if(tag_id){
				...
				$.post("<?php echo get_uri("hr_profile/job_position_delete_tag_item/") ?>"+tag_id).done(function(response) {
		...
		initDataTable(table_job_position, "<?php echo get_uri("hr_profile/table_job_position") ?>", [0], [0], jobPositionServerParams, [0, 'desc']);
	...
	function hrrecord_edit_job_position(invoker,id){
		...
		$.post("<?php echo get_uri("hr_profile/get_list_job_position_tags_file/") ?>"+id).done(function(response) {
			response = JSON.parse(response);
	...
	function view_hr_profilestaff_file_job_position(id, rel_id) {   
		...
		$("#contract_file_data").load("<?php echo get_uri("hr_profile/preview_job_position_file/") ?>" + id + '/' + rel_id, function(response, status, xhr) {
	...
	function delete_job_position_attachment(wrapper, id) {
		...
		if (confirm_delete()) {
			...
			$.get("<?php echo get_uri("hr_profile/delete_hr_profile_job_position_attachment_file/") ?>" + id, function (response) {
