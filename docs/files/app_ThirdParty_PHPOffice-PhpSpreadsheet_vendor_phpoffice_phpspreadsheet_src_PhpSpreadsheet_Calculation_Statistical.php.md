...
- Path: `app\ThirdParty\PHPOffice-PhpSpreadsheet\vendor\phpoffice\phpspreadsheet\src\PhpSpreadsheet\Calculation\Statistical.php`
- Type: PHP
- Size: 56970 bytes
...
- `BETADIST($value, $alpha, $beta, $rMin = 0, $rMax = 1)`
- `BETAINV($probability, $alpha, $beta, $rMin = 0, $rMax = 1)`
- `BINOMDIST($value, $trials, $probability, $cumulative)`
- `CHIDIST($value, $degrees)`
...
- `FDIST2($value, $u, $v, $cumulative)`
- `FISHER($value)`
- `FISHERINV($value)`
- `FORECAST($xValue, $yValues, $xValues)`
- `GAMMAFunction($value)`
- `GAMMADIST($value, $a, $b, $cumulative)`
...
- `HARMEAN(...$args)`
- `HYPGEOMDIST($sampleSuccesses, $sampleNumber, $populationSuccesses, $populationNumber)`
...
- `LOGNORMDIST2($value, $mean, $stdDev, $cumulative = false)`
- `MAX(...$args)`
- `MAXA(...$args)`
- `MAXIFS(...$args)`
- `MEDIAN(...$args)`
